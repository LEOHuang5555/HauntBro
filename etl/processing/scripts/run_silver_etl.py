...
from etl.processing.database_orm import etl_db
...
class SilverETLProcessor:
    ...
    async def initialize_db(self):
        ...
        if not await etl_db.initialize():
            raise RuntimeError("Failed to initialize database ORM manager")
        return etl_db
        ...
        try:
            ...
            if quality_score < 0.3:
                ...
                await etl_db.update_processing_status(
                    story_id=story_id,
                    status='skipped',
                    metadata={'reason': 'low_quality', 'quality_score': quality_score}
                )
            ...
            if not chunks:
                ...
                await etl_db.update_processing_status(
                    story_id=story_id,
                    status='failed',
                    metadata={'reason': 'no_chunks', 'quality_score': quality_score}
                )
            ...
            for chunk in chunks:
                ...
                # Prepare chunk data for ORM
                chunk_data = {
                    'chunk_text': chunk['text'],
                    'chunk_context': story['content'][:200] + '...',
                    ...
                    'chunk_quality_score': 0.8,  # Default quality
                    'embedding_model': 'simple_processing',
                    'embedding_version': '1.0',
                    'embedding_model_version': 'run_silver_etl_v1'
                }
                
                # Save to silver layer using ORM
            ...
            # Update processing status using ORM
    ...
    async def run_pipeline(self, limit: int = None):
        ...
        try:
            ...
            # Get unprocessed bronze stories using ORM
            bronze_stories = await etl_db.get_unprocessed_bronze_stories(
                limit=limit,
            ...
            for i, story in enumerate(stories, 1):
                ...
                result = await self.process_story(dict(story))
